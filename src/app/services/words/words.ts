...
import { PageResponse, UserStatsSummaryDTO, WordsDTO, WordStatisticsDTO } from '../../models/words/words.model';
...
export class Words {
  ...
  // ~TranslationController
  // Synonyms (WordController)
 
  addSynonym(wordId: string, synonymId: string) {
    return this.http.post(`${this.apiUrl}/words/${wordId}/synonyms/${synonymId}`, {});
  }
 
  removeSynonym(wordId: string, synonymId: string) {
    return this.http.delete(`${this.apiUrl}/words/${wordId}/synonyms/${synonymId}`);
  }
  // ~Synonyms
